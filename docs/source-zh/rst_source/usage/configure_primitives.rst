...
Planner 决定执行什么操作，而 **动作原语** 决定如何执行。每个动作原语
...
- **脚本化动作原语**：用于执行 ``move_to``、``rotate_wrist``、
  ``release`` 和 ``back_project`` 等确定性动作。这类动作原语位于
...
WAM (World Action Model)、Diffusion Policy 或 MPC 动作原语 ——
